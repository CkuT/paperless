...
class Document(models.Model):
    ...
    slugified_content = models.TextField(
        db_index=True,
        blank=True,
        editable=False,
    )

    slugified_title = models.CharField(
        max_length=128,
        blank=True,
        db_index=True,
        editable=False,
    ...
    def save(self, *args, **kwargs):
        if self.title is not None:
            self.slugified_title = slugifyOCR(self.title)
        if self.content is not None:
            self.slugified_content = slugifyOCR(self.content)
